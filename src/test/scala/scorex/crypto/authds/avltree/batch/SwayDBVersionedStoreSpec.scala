...
class SwayDBVersionedStoreSpec extends AnyPropSpec
  ...
  val storeTest: SwayDBVersionedStore => Unit = { store =>
    ...
    forAll { b: Array[Byte] =>
      val pair = (Blake2b256(0.toByte +: version.map(v => v.mapKey.digest.head).getOrElse(0: Byte) +: b),
        Blake2b256(version.map(_.mapKey.digest.head).getOrElse(0: Byte) +: b))
      keys += pair
      // Versioned storage is implemented differently, so lets just place in a random long
      val nextVersion = VersionedDigest(Longs.toByteArray((Math.random() * 1000000L).toLong), version.map(v => v.mapKey.versionNum).getOrElse(0L) + 1)
      store.update(nextVersion.digest, Seq(), Seq(pair).map(p => PlasmaKey(p._1) -> PlasmaVal(p._2))).get

      if (version.isDefined) {
        store.rollbackTo(version.get.mapKey.digest)
        store.update(nextVersion.digest, Seq(), Seq(pair).map(p => PlasmaKey(p._1) -> PlasmaVal(p._2))).get
      }
      version = store.getVersion(nextVersion.digest).map(v => v.get)
